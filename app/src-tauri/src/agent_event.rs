...
/// T24b 规则 A：`file_change` 的 `summary` 只留 basename（可读性），真实全路径本来
/// 丢了——前端 `imageArtifacts.ts` 靠「含目录分隔符 + 图片后缀」的形状识别产物，basename
/// 过不了形状检查。这里把 `output`（file_change 从不产生文本输出，一直是 `None`）复用
/// 成「本次改动里识别为图片的完整路径」，换行分隔——前端既有的 summary+output 通用扫描
/// 就能捞到，不必新增字段、不必碰 db::Block::Tool。
