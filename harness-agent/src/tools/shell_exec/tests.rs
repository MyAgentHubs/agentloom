...
fn shell_tool_description_explains_cmd_dialect() {
    let definition = shell_tool_definition_for_dialect(crate::exec::controlled::ShellDialect::Cmd);
    let description = definition["function"]["description"].as_str().unwrap();
...
async fn shell_exec_timeout_stays_recoverable_with_actionable_error() {
    ...
    assert!(error.contains("Running the same command again unchanged will likely time out again"));
