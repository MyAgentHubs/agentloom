...
pub struct PlanRunOptions {
    ...
    /// 显式放行的额外只读根（CLI `--read-root`，可重复）。透传给每个 child run。
    ...
    pub images: Vec<crate::image::ImageBlock>, // CLI --image 附件·挂 planner goal·不支持时诚实降级
}
...
async fn plan_worklist<P: ProviderClient>(
    ...
    let mut messages = images::seed_messages(PLANNER_SYSTEM, &opts.objective, opts.images.clone());
    crate::image::degrade_unsupported_images(&mut messages, &provider.capabilities(), recorder)?;
    for attempt in 0..k {
...
fn child_run_options(
    ...
    RunOptions {
        ...
        extra_read_roots: opts.extra_read_roots.clone(),
        ...
        images: Vec::new(), // 顶层 goal 图片只挂 planner 消息（见 plan_worklist）；child 不重发
    }
}
